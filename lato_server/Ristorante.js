...
var pizze = require("./Listino_pizze");
var bibite = require("./Listino_bibite");
var user = require("./User");
var prodotti_ordinati = require("./Prodotti_ordinati");
var ordini = require("./Ordini");
var aggiunte = require("./Listino_aggiunte");
var tavoli = require("./Elenco_tavoli");
...
app.get("/listino_bibite", function (req, res) {
    bibite
        .getModel()
        .find()
        .sort({
...
app.get("/listino_aggiunte_pizze", function (req, res) {
    aggiunte
        .getModel()
        .find({
        pizza_bibita: true
...
app.get("/listino_aggiunte_bibite", function (req, res) {
    aggiunte
        .getModel()
        .find({
        pizza_bibita: false
...
app.get("/elenco_tavoli", function (req, res) {
    tavoli
        .getModel()
        .find()
        .sort({
...
app.get("/users_camerieri", auth, function (req, res, next) {
...
app.get("/users_baristi", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        user.getModel()
            .find({
            ruolo: "Barista"
...
app.get("/users_pizzaioli", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        user.getModel()
            .find({
            ruolo: "Pizzaiolo"
...
app.get("/users_cassieri", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        user.getModel()
            .find({
            ruolo: "Cassiere"
...
app.post("/crea_utente", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
...
app["delete"]("/elimina_utente/:id", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        user
            .getModel()
            .deleteOne({
...
app.post("/crea_ordine", auth, function (req, res, next) {
...
app.post("/aggiungi_prodotto/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Cameriere") {
...
app.post("/chiudi_ordine/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Cameriere") {
        var pizze_trovate = 2;
        ...
        prodotti_ordinati
            ...
            .then(function (data) {
            for (var i = 0; (i < data.length) && !(pizze_trovate == 0 && bibite_trovate == 0); i++)
...
app.get("/prossima_ord_barista", auth, function (req, res, next) {
...
app.post("/abbandona_ordine_bibite/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Barista") {
        ordini
            .getModel()
            .find({
...
app.post("/bibite_pronte/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Barista") {
        ordini
            .getModel()
            .find({
...
app.get("/prossima_ord_pizzaiolo", auth, function (req, res, next) {
...
app.post("/pizza_inizio_preparazione/:id_prodotto", auth, function (req, res, next) {
    if (req.user.ruolo == "Pizzaiolo") {
        prodotti_ordinati
            .getModel()
            .updateOne({
...
app.post("/pizza_fine_preparazione/:id_prodotto", auth, function (req, res, next) {
    if (req.user.ruolo == "Pizzaiolo") {
        prodotti_ordinati
            .getModel()
            .updateOne({
...
app.post("/pizze_pronte/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Pizzaiolo") {
        ordini
            .getModel()
            .find({
...
app.get("/ordini_da_consegnare", auth, function (req, res, next) {
    if (req.user.ruolo == "Cameriere") {
...
app.post("/bibite_consegnate/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Cameriere") {
        ordini
            .getModel()
            .updateOne({
...
app.post("/pizze_consegnate/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Cameriere") {
        ordini
            .getModel()
            .updateOne({
...
app.get("/tavoli_occupati", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        ordini
            .getModel()
            .find({
...
app.get("/informazioni_ordine/:numero_tavolo", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
...
app.get("/informazioni_prodotti/:id_ordine", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        prodotti_ordinati
            .getModel()
            .find({
...
app.get("/calcolo_totale/:numero_tavolo", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        ordini
            .getModel()
            .findOne({
...
app.post("/pagamento_ordine/:numero_tavolo", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        ordini
            .getModel()
            .updateOne({
...
app.get("/incasso_giorno", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        var d = new Date();
...
app.get("/incasso_settimana", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        var d = new Date();
...
app.get("/incasso_mese", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        var d = new Date();
...
app.get("/statistiche_dipendente/:id", auth, function (req, res, next) {
    if (req.user.ruolo == "Cassiere") {
        var result = {};
...
//
mongoose.connect("mongodb://localhost:27017/ristorante").then(function onconnected() {
            ...
            w.save().then(function () {
                ...
                x.save().then(function () {
                    ...
                    pizze //da vedere con thomas questa parte
                        ...
                        .then(function (count) {
                        if (count == 0) {
                            console.log("Sto aggiungendo alcuni prodotti al database");
    ...
    server.listen(80, function () { return console.log("Server HTTP in ascolto sulla porta 80"); });
